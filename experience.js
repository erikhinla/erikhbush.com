...
function mountInfra(){
 ...
 const home=onLife?'/life':(onBiz?'/':'#arrival');
 const life=onLife?'#daily':'/life';
 const proof=onLife?'/bbai':'#proof';
 const me=onLife?'#founder':'/life';
...
const gateCopy={out:'Consume and be consumed by AI tools without learning them and get passed by those who did.',back:'Continue Managing Digital Fog as AI\'s assistant, always busy but not building something scalable that moves you.',up:'It was never ours to carry. One door. Find the stall.'};
const gateUpHtml=document.body.dataset.page==='life'
  ?'It was never ours to carry.<br>One door. Find the stall.'
  :'It was never ours to carry.<br>Two doors. Find the stall, or find the&nbsp;friction.';
...
function start(lane){closeAll();const onBiz=document.body.dataset.page==='business';const onLife=document.body.dataset.page==='life';if(lane==='personal'&&onBiz){location.href='/life?start=scan';return;}if(lane==='business'&&onLife){location.href='/bbai?start=map';return;}if(!state.gate&&!onBiz){if($('#gate')){go('#gate');return;}}if(!state.gate)setStood(true);setLane(lane);state.lane=lane;state.step=0;const qs=window.REVIEW_QUESTIONS?.[lane];if(!qs){if(lane==='business')open('map-intro');return;}state.selections=Array(qs.length).fill(null);if(lane==='business')open('map-intro');else{renderQuestion();open('questionnaire');}}
